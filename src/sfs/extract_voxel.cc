...
void UpdateOnSurface(ugu::VoxelGrid* voxel_grid) {
  ...
  for (int z = 0; z < voxel_num.z(); z++) {
    for (int y = 0; y < voxel_num.y(); y++) {
      for (int x = 1; x < voxel_num.x(); x++) {
        const ugu::Voxel& prev_voxel = voxel_grid->get(x - 1, y, z);
        ugu::Voxel* voxel = voxel_grid->get_ptr(x, y, z);
  ...
  for (int z = 0; z < voxel_num.z(); z++) {
    for (int x = 0; x < voxel_num.x(); x++) {
      for (int y = 1; y < voxel_num.y(); y++) {
        const ugu::Voxel& prev_voxel = voxel_grid->get(x, y - 1, z);
        ugu::Voxel* voxel = voxel_grid->get_ptr(x, y, z);
        if (voxel->update_num < 1 || prev_voxel.update_num < 1) {
          continue;
        }
  ...
  for (int y = 0; y < voxel_num.y(); y++) {
    for (int x = 0; x < voxel_num.x(); x++) {
      for (int z = 1; z < voxel_num.z(); z++) {
        const ugu::Voxel& prev_voxel = voxel_grid->get(x, y, z - 1);
        ugu::Voxel* voxel = voxel_grid->get_ptr(x, y, z);
        if (voxel->update_num < 1 || prev_voxel.update_num < 1) {
          continue;
        }
  ...
  for (int z = 0; z < voxel_num.z(); z++) {
    for (int y = 0; y < voxel_num.y(); y++) {
      ...
      bool found_min{false};
      {
        ...
        for (int x = 0; x < voxel_num.x(); x++) {
          const ugu::Voxel& voxel = voxel_grid->get(x, y, z);
